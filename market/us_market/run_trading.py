...
def check_us_market_hours():
    """US 장 운영 시간 확인 (09:30 ~ 16:00 EST/EDT, 평일)"""
    # 미국 동부 시간 (EST/EDT) - pytz 라이브러리 사용 권장하나 여기선 간단히 구현
    # UTC-5 (EST) 또는 UTC-4 (EDT). 썸머타임은 3월 둘째주 일요일 ~ 11월 첫째주 일요일
    # 여기서는 간단히 pytz 없이 UTC 기준으로 계산하거나, 시스템 시간을 믿고 9:30~16:00 체크
    
    # 1. UTC 시간 가져오기
    now_utc = datetime.now(timezone.utc)
    
    # 2. 미국 동부 시간 변환 (간단히 UTC-4로 가정 - 썸머타임 기간)
    # 정교하게 하려면 pytz 필요: pip install pytz
    # 여기서는 대략적으로 UTC-4 (EDT) 적용
    ...
    return False, f"장 마감 (현재 US: {current_time.strftime('%H:%M:%S')})"
...
def main():
    ...
    while not _stopped:
        # 0. 장 운영 시간 체크 (US)
        is_open, msg = check_us_market_hours()
        if not is_open:
