...
def ensure_column_exists(conn, table_name, column_name, column_type):
    ...
    if column_name not in columns:
        ...
        # print(f"[{datetime.now()}] '{column_name}' 컬럼 추가 완료 ({table_name})")  # 제거됨

def validate_data(df):
    """데이터 검증"""
    # 필수 컬럼 검증
    required_cols = ['timestamp', 'coin', 'interval', 'open', 'high', 'low', 'close', 'volume']
    missing_cols = [col for col in required_cols if col not in df.columns]
    
    if missing_cols:
        # print(f"⚠️ 필수 컬럼 누락: {missing_cols}")  # 제거됨
        return False
    
    # 값 범위 검증
    if (df['high'] < df['low']).any():
        # print("⚠️ high가 low보다 작은 값 발견")  # 제거됨
        return False
    
    if (df['volume'] < 0).any():
        # print("⚠️ 음수 거래량 발견")  # 제거됨
        return False
...
def add_technical_and_wave_indicators(df, interval: str = None):
    ...
    plus_di = pd.Series(np.where(di_sum > 0, 100 * plus_di / di_sum, 0))
    minus_di = pd.Series(np.where(di_sum > 0, 100 * minus_di / di_sum, 0))
    dx = 100 * np.abs(plus_di - minus_di) / (plus_di + minus_di + 1e-9)
    df['adx'] = np.clip(dx.rolling(14).mean(), 0, 100)
    ...
    print(f"    ✅ {interval}: 33개 핵심 컬럼 계산 완료 (총 {len(df)}개 row)")
    return df

# 🚀 30개 컬럼에 포함되지 않는 함수들은 제거됨

# 🚀 30개 컬럼에 포함되지 않는 빠른 지표 계산 함수들은 제거됨

# Zigzag 계산 함수 (rl_candles_calculate.py와 동일한 방식)
def add_zigzag(df: pd.DataFrame, interval: str, percent: float = None) -> pd.DataFrame:
    """
    🚀 최적화된 Zigzag 계산 - 방향성 + 전환점 가격 중심 방식 (rl_candles_calculate.py와 통일)
    ...
    # ✅ 인터벌별 Lookback 캔들 수 설정 (rl_candles_calculate.py와 동일)
    lookback_map = {
        # 5m 제거
        '15m': 3,   # 약 45분간 기준
        '30m': 3,   # 약 1.5시간 기준
    ...
    # print(f"    📏 Zigzag 슬라이딩 윈도우: {lookback}개 캔들 기준 (인터벌: {interval})")  # 제거됨
    ...
    # print(f"    🎯 슬라이딩 윈도우 계산 시작: 총 {len(close)}개 캔들, Lookback: {lookback}개")  # 제거됨
    ...
    for i in range(lookback, len(close) - lookback):
        ...
        if center == window_max:
            ...
            change_count += 1
            # print(f"    🔺 고점 전환점 발견: {center:.4f} (캔들 {i}, 윈도우: {window_start}~{window_end-1})")  # 제거됨
        ...
        # ✅ 저점 전환점 판단 (현재가가 윈도우 내 최저가)
            ...
            zz_pivot_price[i] = center  # 전환점 가격 저장
            change_count += 1
            # print(f"    🔻 저점 전환점 발견: {center:.4f} (캔들 {i}, 윈도우: {window_start}~{window_end-1})")  # 제거됨
    ...
    # print(f"    🔄 Zigzag 전환점 발견: {change_count}개")  # 제거됨
    # print(f"    ✅ 최종 Zigzag: 방향성 {len(zz_direction)}개, 유효 전환점: {sum(1 for d in zz_direction if d != 0)}개")  # 제거됨
    
    # ✅ 전환점 부족 → 최소 1개 이상 필요 (유효성 검증 기준 완화)
    if change_count < 1:
        # print(f"    ⚠️ {interval}: Zigzag 전환점 부족 (1개 미만) - 대체 계산 방식 사용")  # 제거됨
    ...
    if non_zero_directions == 0:
        ...
        if len(close) >= 2:
            ...
            change_count = 2
            # print(f"    🔄 기본 전환점 생성: 첫 번째(고점), 마지막(저점)")  # 제거됨
    
    # print(f"    ✅ {interval}: 슬라이딩 윈도우 Zigzag 계산 정상 - 방향변화: {non_zero_directions}개")  # 제거됨
...
def calculate_wave_numbers(df):
    ...
    # ✅ 최소 데이터 수 검증
    if len(df) < 10:
        # print("    ⚠️ 데이터 수 부족 - wave_number 계산 생략")  # 제거됨
        return pd.Series(0, index=df.index)
    
    # ✅ zigzag_direction 데이터 유효성 확인
    if 'zigzag_direction' not in df.columns:
        # print("    ⚠️ zigzag_direction 컬럼 없음 - wave_number 계산 생략")  # 제거됨
        return pd.Series(0, index=df.index)
    ...
    if non_zero_directions == 0:
        # print("    ⚠️ zigzag_direction이 모두 0 - wave_number 전부 0으로 처리")  # 제거됨
        return pd.Series(0, index=df.index)
    ...
    # ✅ 파동 번호 유효성 확인
    if wave_count == 0:
        # print("    ⚠️ 파동 번호 계산 실패 - 모든 값 0으로 설정")  # 제거됨
        return pd.Series(0, index=df.index)
    elif wave_count < 3:  # 최소 3개 파동
        # print(f"    ⚠️ 파동 번호 부족 ({wave_count}개) - 분석 제한적이지만 저장 진행")  # 제거됨
        pass
    else:
        # print(f"    ✅ 파동 번호 계산 정상 - 최대 파동: {wave_count}개")  # 제거됨
        pass
        pass
...
def calculate_wave_progress(df):
    ...
    if len(df) < 5:  # 더 작은 데이터셋에서도 작동하도록 조정
        # print("    ⚠️ 데이터 수 부족 - wave_progress 계산 생략")  # 제거됨
        return pd.Series(0.5, index=df.index)  # 기본값 0.5로 변경
    
    # ✅ zigzag_direction 데이터 유효성 확인
    if 'zigzag_direction' not in df.columns:
        # print("    ⚠️ zigzag_direction 컬럼 없음 - 대체 계산 방식 사용")  # 제거됨
    ...
    non_zero_directions = (zigzag_direction != 0).sum()
    if non_zero_directions == 0:
        # print("    ⚠️ zigzag_direction이 모두 0 - 대체 계산 방식 사용")  # 제거됨
        return _calculate_wave_progress_fallback(df)
    ...
    if progress_mean == 0:
        # print("    ⚠️ 파동 진행률 계산 실패 - 대체 계산 방식 사용")  # 제거됨
        return _calculate_wave_progress_fallback(df)
    elif progress_mean < 0.01:  # 평균 0.01 이상 (기준 완화)
        # print(f"    ⚠️ 파동 진행률 낮음 (평균: {progress_mean:.3f}) - 대체 계산 방식 사용")  # 제거됨
        return _calculate_wave_progress_fallback(df)
    else:
        # print(f"    ✅ 파동 진행률 계산 정상 - 평균: {progress_mean:.3f}")  # 제거됨
        pass
...
def _calculate_wave_progress_fallback(df):
    ...
    try:
        ...
        # 🚀 가격 기반 파동 진행률 계산 (슬라이딩 윈도우) (우선순위 2)
            ...
            # print("    ✅ 가격 기반 파동 진행률 계산 완료")  # 제거됨
            return wave_progress.fillna(0.5)
        ...
        # 🚀 MACD 기반 파동 진행률 계산 (우선순위 3)
            ...
            # print("    ✅ MACD 기반 파동 진행률 계산 완료")  # 제거됨
            return wave_progress
        
        else:
            # print("    ⚠️ 대체 계산 불가능 - 기본값 0.5 사용")  # 제거됨
            return pd.Series(0.5, index=df.index)
            
    except Exception as e:
        # print(f"    ⚠️ 대체 계산 중 오류: {e} - 기본값 0.5 사용")  # 제거됨
        return pd.Series(0.5, index=df.index)

def _calculate_pattern_confidence_fallback(df):
    """패턴 감지 실패 시 대체 패턴 신뢰도 계산 (단일 값 반환)"""
    try:
        if len(df) == 0:
            return 0.0
...
def _calculate_pattern_quality_fallback(df):
    ...
    try:
        ...
        # print("    ✅ 대체 패턴 품질 계산 완료")  # 제거됨
        return pd.Series(quality_scores, index=df.index)
        
    except Exception as e:
        # print(f"    ⚠️ 대체 패턴 품질 계산 오류: {e} - 기본값 0.5 사용")  # 제거됨
        return pd.Series(0.5, index=df.index)
...
def _calculate_pattern_type_fallback(df):
    ...
    try:
        ...
        # print("    ✅ 대체 패턴 타입 계산 완료")  # 제거됨
        return pd.Series(pattern_types, index=df.index)
        
    except Exception as e:
        # print(f"    ⚠️ 대체 패턴 타입 계산 오류: {e} - 기본값 'unknown' 사용")  # 제거됨
...
def compute_wave_step(df: pd.DataFrame) -> pd.Series:
    ...
    for i in range(len(df)):
        ...
        # 🚀 추가 보정 요소들
        # 파동 번호 기반 보정 (wave_number는 33개 컬럼에서 제거됨 - 기본값 사용)
        wave_num = 1  # 기본값
        if wave_num > 5:
    ...
    for i in range(len(df) - window_size + 1):
        ...
        if len(highs) >= 3:
            ...
            if (head > left_shoulder and head > right_shoulder and
                ...
                pattern_quality = (
                    min(window_df[window_df['pivot_high'] == 1]['pivot_strength'].iloc[:3]) * 0.4 +
                    min(window_df[window_df['pivot_high'] == 1]['volume'].iloc[:3] / window_df['volume'].mean()) * 0.3 +
                ...
                patterns.append({
                    ...
                    'pattern_volume_ratio': min(window_df[window_df['pivot_high'] == 1]['volume'].iloc[:3] / window_df['volume'].mean()),
                    'pattern_pivot_strength': min(window_df[window_df['pivot_high'] == 1]['pivot_strength'].iloc[:3]),
                })
        ...
        if len(highs) >= 2:
            ...
            if abs(high1 - high2) / high1 < 0.1:
                pattern_quality = (
                    min(window_df[window_df['pivot_high'] == 1]['pivot_strength'].iloc[:2]) * 0.4 +
                    min(window_df[window_df['pivot_high'] == 1]['volume'].iloc[:2] / window_df['volume'].mean()) * 0.3 +
                    (1 - window_df['close'].pct_change().std()) * 0.3
                )
                ...
                patterns.append({
                    ...
                    'pattern_start_idx': window_df.index[0],
                    'pattern_end_idx': window_df.index[-1],
                    'pattern_volume_ratio': min(window_df[window_df['pivot_high'] == 1]['volume'].iloc[:2] / window_df['volume'].mean()),
                    'pattern_pivot_strength': min(window_df[window_df['pivot_high'] == 1]['pivot_strength'].iloc[:2]),
                })
...
def process_interval_data(interval, table_name):
    ...
    essential_columns = [
        ...
        # 🚀 레짐 분석 (4개)
        ('regime_stage', 'INTEGER'), ('regime_label', 'TEXT'), ('regime_confidence', 'REAL'), ('regime_transition_prob', 'REAL')
    ]
    ...
    # 🚀 코인 목록을 배치로 가져오기
    coins = pd.read_sql(f'SELECT DISTINCT coin FROM {table_name} WHERE interval = ?', conn, params=(interval,))['coin'].tolist()
    
    # 🚀 배치 처리 크기 설정
    batch_size = 50  # 한 번에 처리할 코인 수
    total_coins = len(coins)
    
    print(f"[{datetime.now()}] 🚀 {interval} 처리 시작: {total_coins}개 코인, 배치 크기: {batch_size}")

    for i in range(0, total_coins, batch_size):
        batch_coins = coins[i:i + batch_size]
        print(f"[{datetime.now()}] 🔄 배치 {i//batch_size + 1}/{(total_coins + batch_size - 1)//batch_size}: {len(batch_coins)}개 코인 처리 중...")
        
        for coin in batch_coins:
            # 🚀 1단계: 가장 최근에 계산된 데이터 찾기
            latest_calculated = pd.read_sql(f'''
                SELECT timestamp FROM {table_name}
                WHERE coin=? AND interval=? AND rsi IS NOT NULL AND macd IS NOT NULL
                ORDER BY timestamp DESC
                LIMIT 1
            ''', conn, params=(coin, interval))
            
            if latest_calculated.empty:
                # 🚀 아무것도 계산되지 않은 경우: 최근 200개 로드 (Zigzag 계산을 위해 충분한 데이터 확보)
                print(f"[{datetime.now()}] 🔄 {coin}-{interval}: 처음 계산 - 최근 200개 데이터 로드")
                df = pd.read_sql(f'''
                    SELECT * FROM {table_name}
                    WHERE coin=? AND interval=?
                    ORDER BY timestamp DESC
                    LIMIT 200
                ''', conn, params=(coin, interval)).sort_values('timestamp').reset_index(drop=True)
            else:
                # 🚀 이미 계산된 데이터가 있는 경우: 최근 계산된 데이터 기준으로 앞 150개 + 새로운 데이터
                latest_timestamp = latest_calculated.iloc[0]['timestamp']
                # print(f"[{datetime.now()}] 🔄 {coin}-{interval}: 증분 업데이트 - 최근 계산: {datetime.fromtimestamp(latest_timestamp)}")  # 제거됨
                
                # 🚀 Zigzag 계산을 위해 충분한 데이터 확보 (최소 150개)
                df = pd.read_sql(f'''
                    SELECT * FROM {table_name}
                    WHERE coin=? AND interval=? AND timestamp <= ?
                    ORDER BY timestamp DESC
                    LIMIT 150
                ''', conn, params=(coin, interval, latest_timestamp)).sort_values('timestamp').reset_index(drop=True)
                ...
                ''', conn, params=(coin, interval, latest_timestamp))
                
                if not new_data.empty:
                    df = pd.concat([df, new_data], ignore_index=True).sort_values('timestamp').reset_index(drop=True)
                    # print(f"[{datetime.now()}] 🔄 {coin}-{interval}: 새로운 데이터 {len(new_data)}개 추가 (총 {len(df)}개)")  # 제거됨
                
                # 🚀 데이터가 여전히 부족한 경우 더 많은 데이터 로드 (Zigzag 계산을 위해 최소 100개 확보)
                if len(df) < 100:
                    # print(f"[{datetime.now()}] ⚠️ {coin}-{interval}: 데이터 부족 ({len(df)}개) - 더 많은 데이터 로드")  # 제거됨
                    df = pd.read_sql(f'''
                        SELECT * FROM {table_name}
                        WHERE coin=? AND interval=?
                        ORDER BY timestamp DESC
                        LIMIT 200
                    ''', conn, params=(coin, interval)).sort_values('timestamp').reset_index(drop=True)
                    # print(f"[{datetime.now()}] 🔄 {coin}-{interval}: 충분한 데이터 로드 완료 (총 {len(df)}개)")  # 제거됨

            if df.empty or len(df) < 20:
                # print(f"[{datetime.now()}] 데이터 부족: {coin}-{interval} (데이터: {len(df)}개)")  # 제거됨
                continue

            # 🚀 2단계: 지표가 계산되지 않은 데이터 필터링
            indicator_columns = ['rsi', 'macd', 'bb_upper', 'mfi', 'wave_phase']
            has_indicators = df[indicator_columns].notna().all(axis=1)
            
            if has_indicators.all():
                # print(f"[{datetime.now()}] ✅ 모든 데이터에 지표가 이미 계산됨: {coin}-{interval}")  # 제거됨
                continue
            
            # 🚀 지표가 없는 데이터만 처리
            new_data_mask = ~has_indicators
            ...
            # print(f"[{datetime.now()}] 🔄 {coin}-{interval}: {new_data_count}개 새로운 데이터에 지표 계산 (총 {len(df)}개 데이터 사용)")  # 제거됨
            
            # 🚀 데이터 검증
            if not validate_data(df):
                # print(f"[{datetime.now()}] 데이터 검증 실패: {coin}-{interval}")  # 제거됨
                continue
            ...
            # 🚀 3단계: 기존 데이터 보존하면서 전략지표만 업데이트
            if new_data_mask.sum() > 0:
                # 🚀 새로운 데이터만 필터링 (기존 OHLCV 데이터는 보존)
                update_df = df[new_data_mask].copy()
                
                # 🚀 저장 성공 여부 확인 (기존 데이터는 삭제하지 않고 업데이트만)
                save_success = save_technical_indicators_immediate(update_df, coin, interval)
                
                if save_success:
                    # print(f"[{datetime.now()}] ✅ {coin}-{interval}: {len(update_df)}개 새로운 데이터 저장 성공")  # 제거됨
                    pass
                else:
                    # print(f"[{datetime.now()}] ❌ {coin}-{interval}: 저장 실패")  # 제거됨
                    pass
            else:
                # print(f"[{datetime.now()}] ✅ {coin}-{interval}: 업데이트할 새로운 데이터 없음")  # 제거됨
                pass
    
    conn.close()

# 🚀 rl_candles_calculate.py의 성공적인 저장 함수 추가
def save_technical_indicators_immediate(df: pd.DataFrame, coin: str, interval: str) -> bool:
    """🚀 계산 완료 즉시 저장 - rl_candles_calculate.py의 성공적인 방식 적용"""
    try:
        if df.empty:
            # print(f"    ⚠️ 저장할 데이터가 없음: {coin}/{interval}")  # 제거됨
            return False
        
        # print(f"    💾 즉시 저장 시작: {coin}/{interval} ({len(df)}개 행)")  # 제거됨
        
        # 📌 소숫점 자리수 통일 적용 (30개 핵심 컬럼) - 4자리로 통일
        rounding_map = {
        ...
        technical_columns = [
            ...
            # 🚀 구조 점수 (1개)
            'structure_score'
            # ✅ integrated 컬럼들(9개)은 integrated 파일에서 처리
            # 'volatility_level', 'risk_level', 'integrated_direction', 'sentiment', 'sentiment_label', 'regime_stage', 'regime_label', 'regime_confidence', 'regime_transition_prob'
        ]
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            if missing_columns:
                # print(f"    🔧 누락된 컬럼 {len(missing_columns)}개 추가: {missing_columns}")  # 제거됨
                for col in missing_columns:
                    # 컬럼 타입 결정
                    if col in ['wave_phase', 'three_wave_pattern', 'sideways_pattern', 'pattern_type', 
                              'pattern_direction', 'pattern_class', 'flow_level_meta']:
                        col_type = 'TEXT'
                    ...
                    try:
                        ...
                        # print(f"    ✅ 컬럼 추가 완료: {col} ({col_type})")  # 제거됨
                    except Exception as e:
                        # print(f"    ⚠️ 컬럼 추가 실패: {col} - {e}")  # 제거됨
                        continue
            ...
            if not existing_columns:
                # print(f"    ⚠️ 저장할 기술지표 컬럼이 없음: {coin}/{interval}")  # 제거됨
                return False
            
            # print(f"    💾 {len(existing_columns)}개 컬럼 업데이트: {existing_columns}")  # 제거됨
                    ...
                    if pd.notna(row[col]):
                        ...
                        if col in ['zigzag_direction', 'pattern_start_idx', 'pattern_end_idx']:
                            try:
                                ...
                                value = 0
                        elif col in ['wave_phase', 'pattern_type', 'volatility_level', 'risk_level', 'integrated_direction', 'sentiment_label', 'regime_label']:
                            # TEXT 타입 컬럼 - 문자열로 변환
        ...
        # print(f"    ✅ 즉시 저장 완료: {coin}/{interval} → {total_updated}개 행 업데이트")  # 제거됨
        
        # 🔍 저장 결과 검증
        if total_updated > 0:
            # print(f"    🔍 저장 검증: {coin}/{interval} - {total_updated}개 행 성공적으로 저장됨")  # 제거됨
            return True
        else:
            # print(f"    ⚠️ 저장 검증: {coin}/{interval} - 업데이트된 행이 없음")  # 제거됨
            return False
        
    except Exception as e:
        print(f"    ❌ 즉시 저장 중 오류: {coin}/{interval} - {str(e)}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    ...
    print(f"[{datetime.now()}] 🚀 기술지표 계산 시작 - {len(intervals)}개 인터벌")
