#!/usr/bin/env python
"""
통합 분석 v1 - 재설계된 계층적 구조 (interval_profiles 통합 버전)
...
class IntegratedAnalyzerV1:
    """
    재설계된 통합 분석기 (interval_profiles 통합)
    ...
    def __init__(self, db_path: str = None):
        ...
        logger.info(f"🎯 통합 분석 v1 초기화 완료")
        logger.info(f"   DB 경로: {self.db_path}")
        logger.info(f"   방향 가중치 (1d/240m): {self.direction_weights}")
        logger.info(f"   타이밍 가중치 (30m/15m): {self.timing_weights}")
