...
def calculate_composite_regime_score(df: pd.DataFrame, interval: str) -> pd.Series:
    ...
    criteria = REGIME_CRITERIA[interval]
...
# ✅ ensure_integrated_columns_exist 함수는 save_integrated_indicators_immediate에서 처리
...
def perform_integrated_analysis(coin: str, interval: str):
    ...
    try:
        ...
        composite_score = calculate_composite_regime_score(df, interval)
        # 1) 점수 평활화(EWM)
        lookback = REGIME_CRITERIA[interval]['lookback_period']
        smooth_score = composite_score.ewm(span=lookback, min_periods=1).mean()
        ...
        df['regime_label'] = [REGIME_STAGES[int(stage)] for stage in df['regime_stage']]
...
def validate_regime_results():
    """레짐 결과 전체 검증 (방식 B: 전체 완료 후 일괄 검증)"""
    try:
        with sqlite3.connect(DB_PATH) as conn:
            # 레짐 컬럼이 있는 데이터 조회
            df = pd.read_sql("""
                SELECT coin, interval, regime_stage, regime_label, 
                       regime_confidence, regime_transition_prob
                FROM candles 
                WHERE regime_stage IS NOT NULL
                ORDER BY coin, interval, timestamp
            """, conn)
        ...
        # 1. 값 범위 검증 (NULL 제외)
        stage_notna = df[df['regime_stage'].notna()]
        conf_notna = df[df['regime_confidence'].notna()]
        trans_notna = df[df['regime_transition_prob'].notna()]
        
        stage_valid = stage_notna.empty or stage_notna['regime_stage'].between(1, 7).all()
        confidence_valid = conf_notna.empty or conf_notna['regime_confidence'].between(0, 1, inclusive='both').all()
        trans_prob_valid = trans_notna.empty or trans_notna['regime_transition_prob'].between(0, 0.4, inclusive='both').all()
        
        # 범위 초과 값 상세 분석 (NULL 제외)
        conf_out_of_range = df[df['regime_confidence'].notna() & ~df['regime_confidence'].between(0, 1, inclusive='both')].copy()
        stage_out_of_range = df[df['regime_stage'].notna() & ~df['regime_stage'].between(1, 7)].copy()
        trans_out_of_range = df[df['regime_transition_prob'].notna() & ~df['regime_transition_prob'].between(0, 0.4, inclusive='both')].copy()
        
        # NULL 비율 계산
        total_count = len(df)
        stage_null = df['regime_stage'].isna().sum()
        label_null = df['regime_label'].isna().sum()
        conf_null = df['regime_confidence'].isna().sum()
        trans_null = df['regime_transition_prob'].isna().sum()
        
        # 통계 정보
        conf_stats = df['regime_confidence'].describe()
        stage_stats = df['regime_stage'].describe()
        trans_stats = df['regime_transition_prob'].describe()
        
        # 2. 레짐 분포 분석
        regime_dist = df['regime_stage'].value_counts().sort_index()
        regime_labels_map = {1: 'extreme_bearish', 2: 'bearish', 3: 'sideways_bearish',
                            4: 'neutral', 5: 'sideways_bullish', 6: 'bullish', 7: 'extreme_bullish'}
        
        # 3. 극단 전환 감지 (1→7 또는 7→1)
        df['regime_change'] = df.groupby(['coin', 'interval'])['regime_stage'].diff().abs()
        extreme_changes = df[df['regime_change'] >= 5].copy()
        
        # 4. 레짐 전환 빈도 계산 (10개 기준으로 5번 이상 전환 - 더 엄격한 기준)
        df['regime_count'] = 0
        for (coin, interval), group in df.groupby(['coin', 'interval']):
            changes = (group['regime_stage'].diff().abs() > 0).astype(int)
            df.loc[group.index, 'regime_count'] = changes.rolling(window=10, min_periods=1).sum()
        
        high_frequency = df[df['regime_count'] >= 5].copy()  # 3→5로 변경 (더 엄격하게)
        
        # 5. 리포트 출력
        print("\n📊 레짐 검증 리포트")
        print("="*60)
        print(f"✅ 커버리지: {total_count:,}개 레짐 데이터")
        print(f"   - regime_stage NULL: {stage_null} ({stage_null/total_count*100:.2f}%)")
        print(f"   - regime_label NULL: {label_null} ({label_null/total_count*100:.2f}%)")
        print(f"   - regime_confidence NULL: {conf_null} ({conf_null/total_count*100:.2f}%)")
        print(f"   - regime_transition_prob NULL: {trans_null} ({trans_null/total_count*100:.2f}%)")
        
        print("\n✅ 값 범위:")
        print(f"   - regime_stage (1~7): {'✅ 정상' if stage_valid else '❌ 범위 초과'}")
        if not stage_valid:
            print(f"      범위 초과: {len(stage_out_of_range)}건")
            print(f"      실제 범위: {stage_stats['min']:.2f} ~ {stage_stats['max']:.2f}")
        
        print(f"   - regime_confidence (0~1): {'✅ 정상' if confidence_valid else '❌ 범위 초과'}")
        if not confidence_valid:
            print(f"      범위 초과: {len(conf_out_of_range)}건")
            print(f"      실제 범위: {conf_stats['min']:.4f} ~ {conf_stats['max']:.4f}")
            if len(conf_out_of_range) > 0 and len(conf_out_of_range) <= 10:
                print(f"      샘플:")
                for idx, row in conf_out_of_range.head(5).iterrows():
                    print(f"         {row['coin']}/{row['interval']}: confidence={row['regime_confidence']:.4f}")
        
        print(f"   - regime_transition_prob (0~0.4): {'✅ 정상' if trans_prob_valid else '❌ 범위 초과'}")
        if not trans_prob_valid:
            print(f"      범위 초과: {len(trans_out_of_range)}건")
            print(f"      실제 범위: {trans_stats['min']:.4f} ~ {trans_stats['max']:.4f}")
        
        print("\n📈 레짐 분포:")
        for stage_num in sorted(regime_dist.index):
            count = regime_dist[stage_num]
            label = regime_labels_map.get(stage_num, 'unknown')
            pct = count / total_count * 100
            print(f"   {stage_num} ({label}): {count:,}건 ({pct:.2f}%)")
        
        # 경고 출력
        warnings = []
        if extreme_changes.shape[0] > 0:
            warnings.append(f"❌ 극단 전환 (5+ 단계): {extreme_changes.shape[0]}건")
        if high_frequency.shape[0] > 0:
            warnings.append(f"⚠️  높은 전환 빈도 (10개 중 5+회): {high_frequency.shape[0]}건")
        if stage_null / total_count > 0.05:
            warnings.append(f"⚠️  stage NULL 비율 높음: {stage_null/total_count*100:.2f}%")
