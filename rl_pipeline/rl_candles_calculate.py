...
def process_interval_data(interval, table_name):
    ...
    # 🚀 배치 처리 크기 설정 (전체 데이터 처리로 인해 배치 크기 축소)
    batch_size = 5  # 한 번에 처리할 코인 수 (전체 데이터 처리로 인해 축소)
    ...
    for i in range(0, total_coins, batch_size):
        ...
        for coin in batch_coins:
            ...
            if latest_calculated.empty:
                # 🚀 아무것도 계산되지 않은 경우: 전체 데이터 로드 (ML 학습을 위해 모든 과거 데이터 필요)
                print(f"[{datetime.now()}] 🔄 {coin}-{interval}: 처음 계산 - 전체 데이터 로드")
                df = pd.read_sql(f'''
                    SELECT * FROM {table_name}
                    WHERE coin=? AND interval=?
                    ORDER BY timestamp ASC
                ''', conn, params=(coin, interval)).reset_index(drop=True)
            else:
                # 🚀 이미 계산된 데이터가 있는 경우: 전체 데이터 로드 (ML 학습을 위해 모든 과거 데이터 필요)
                latest_timestamp = latest_calculated.iloc[0]['timestamp']
                print(f"[{datetime.now()}] 🔄 {coin}-{interval}: 증분 업데이트 - 전체 데이터 로드")
                
                # 🚀 ML 학습을 위해 전체 데이터 로드 (과거부터 현재까지)
                df = pd.read_sql(f'''
                    SELECT * FROM {table_name}
                    WHERE coin=? AND interval=?
                    ORDER BY timestamp ASC
